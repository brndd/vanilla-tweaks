...
Usage: {cmd} path/to/WoW.exe");
    ...
    let quickloot_reverse_find: Vec::<u8> = vec![0x50, 0xba, 0x48, 0xf9, 0x85, 0x00, 0xb9, 0x10, 0x00, 0x00, 0x00, 0xe8, 0xd0, 0xfd, 0xe6, 0xff, 0x85, 0xc0, 0x74, 0x10, 0x8b, 0x4d, 0x08, 0x85, 0xc9, 0x0f, 0x95, 0xc1, 0x51, 0x8b, 0xc8, 0xe8];
    let quickloot_reverse_repl: Vec::<u8> = vec![0x50, 0xba, 0x48, 0xf9, 0x85, 0x00, 0xb9, 0x10, 0x00, 0x00, 0x00, 0xe8, 0xd0, 0xfd, 0xe6, 0xff, 0x85, 0xc0, 0x74, 0x10, 0x8b, 0x4d, 0x08, 0x85, 0xc9, 0x0f, 0x94, 0xc1, 0x51, 0x8b, 0xc8, 0xe8];

    print!("Applying patch: quickloot reverse...");
    match replace(&mut file, quickloot_reverse_find, quickloot_reverse_repl) {
        true => println!(" Success!"),
        false => {
            println!(" FAILED!");
            return ExitCode::from(1);
